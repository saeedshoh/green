<x-dashboard-layout title="Сотрудники">
    <x-dashboard-header-large pretitle="Обзор" title="Сотрудники">
        <a href="{{ route('employees.create') }}" class="btn btn-primary lift">Добавить</a>
            ...
            <div class="table-responsive">
                <table class="table table-hover tabl33e-nowrap card-table">
                    ...
                    <tbody class="list font-size-base">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end">
                                        @if ($user->deleted_at == null)
                                            <a href="{{ route('employees.edit', $user->id) }}" class="btn btn-sm btn-white me-2" data-bs-toggle="tooltip" title="Редактировать">
                                                <span class="fe fe-edit-2"></span>
                                            </a>

                                            <form action="{{ route('employees.destroy', $user->id) }}" id="item{{ $user->id }}" method="POST">
                                                @csrf
                                                @method ('DELETE')
                                        ...
                                        @else
                                            <form action="{{ route('employees.restore', $user->id) }}" id="item{{ $user->id }}" method="POST">
                                                @csrf
                                                @method ('PUT')
                        ...
                        @empty
                            <tr>
                                <td colspan="4"><span class="text-danger">Вы еще не добавили сотрудников</span></td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
